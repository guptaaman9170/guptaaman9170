"""
Convert Aman's portrait photo into a clean monochrome ASCII-art SVG
with precise facial feature extraction that types itself in like a terminal.

Output: aman-ascii.svg
"""
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "aman-ascii.svg")
...
parts.append(f'<text x="{CANVAS_W/2}" y="{TITLEBAR_H/2 + 4}" fill="{TITLE_TEXT}" font-size="12" '
             f'text-anchor="middle">aman@github: ~$ ./portrait.sh</text>')
...
prompt_text = "aman@github:~$ whoami "
name_text = "Aman Kumar Gupta"
cursor_x = PAD + int((len(prompt_text) + len(name_text)) * 7.8) + 4
parts.append(f'<line x1="0" y1="{status_line_y:.1f}" x2="{CANVAS_W}" y2="{status_line_y:.1f}" stroke="{FRAME}"/>')
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{prompt_text}<tspan fill="{INK}" font-weight="700">{name_text}</tspan></text>')
